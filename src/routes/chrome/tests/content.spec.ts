...
describe('/chrome/content API', function () {
  ...
  it('cancels request when they are closed early', async () => {
    ...
    const promise = fetch('http://localhost:3000/chrome/content', {
      body: JSON.stringify(body),
      headers: {
        'content-type': 'application/json',
      },
  ...
  it('404s GET requests', async () => {
    ...
    await fetch('http://localhost:3000/chrome/content?token=browserless').then(
      (res) => {
        expect(res.headers.get('content-type')).to.equal(
  ...
  it('times out requests', async () => {
    ...
    await fetch(
      'http://localhost:3000/chrome/content?token=browserless&timeout=10',
      {
        body: JSON.stringify(body),
        headers: {
          'content-type': 'application/json',
  ...
  it('allows requests without token when auth token is not set', async () => {
    ...
    await fetch('http://localhost:3000/chrome/content', {
      body: JSON.stringify(body),
      headers: {
        'content-type': 'application/json',
      },
